...
import { talentos as dadosPlanilhaTalentos } from "./talentos";
import {
  PONTOS_DISPONIVEIS,
  VALOR_MINIMO,
...
const FichaCJphant = () => {
  ...
  // Estados para ancestralidades (dados da planilha)
  const [ancestralidades, setAncestralidades] = useState([]);
  const [ancestralidadesAbertas, setAncestralidadesAbertas] = useState({});
  ...
  // Estados para Origens (dados da planilha)
  const [origens, setOrigens] = useState([]);
  const [origensAbertas, setOrigensAbertas] = useState({});

  // Estados para Talentos (dados da planilha)
  const [talentos, setTalentos] = useState([]);
  const [talentosAbertos, setTalentosAbertos] = useState({});
  ...
  // EFEITO PARA CALCULAR E ATUALIZAR OS BÔNUS DAS HABILIDADES - CORRIGIDO
  useEffect(() => {
    ...
    habilidades.forEach((hab) => {
      if (hab.trancada) {
        const bonus = calcularBonusHabilidade(
          hab,
          atributos,
          pontosAtributoAncestralidade
        );
        novosBonus[hab.id] = bonus;

        // Verificar se o bônus mudou
        if (bonusHabilidades[hab.id] !== bonus) {
    ...
    if (precisaAtualizar) {
      actions.updateBonusHabilidades(novosBonus);
    }
  }, [habilidades, atributos, pontosAtributoAncestralidade, bonusHabilidades]); // CORREÇÃO: Removido actions da dependência
  ...
  /* console.log(
      ...
      agrupado[item.Ancestralidade].push({
        nome: item.Nome,
  ...
  useEffect(() => {
    // Agrupar por tipo
    const agrupado = {};
    dadosPlanilhaOrigens.forEach((item) => {
      if (!agrupado[item.Tipo]) {
        agrupado[item.Tipo] = [];
      }
      agrupado[item.Tipo].push({
        nome: item.Nome,
        descricao: item.Descrição,
    ...
    const dadosProcessados = Object.keys(agrupado).map((tipo) => ({
      tipo,
      opcoes: agrupado[tipo],
    }));

    setOrigens(dadosProcessados);

    const abertasInicial = {};
    dadosProcessados.forEach((origem) => {
      abertasInicial[origem.tipo] = false;
    });
    setOrigensAbertas(abertasInicial);
  }, []);

  // Processar dados da planilha de Talentos
  useEffect(() => {
    // Agrupar por tipo
    const agrupado = {};
    dadosPlanilhaTalentos.forEach((item) => {
      if (!agrupado[item.Tipo]) {
        agrupado[item.Tipo] = [];
      }
      agrupado[item.Tipo].push({
    ...
    const dadosProcessados = Object.keys(agrupado).map((tipo) => ({
      tipo,
      talentos: agrupado[tipo],
    }));

    setTalentos(dadosProcessados);

    const abertasInicial = {};
    dadosProcessados.forEach((categoria) => {
      abertasInicial[categoria.tipo] = false;
    });
    setTalentosAbertos(abertasInicial);
  }, []);
  ...
  const trancarHabilidade = (id) => {
    ...
    // Calcular o bônus quando a habilidade é trancada
    const habilidadeTrancada = novasHabilidades.find((hab) => hab.id === id);
    if (habilidadeTrancada && habilidadeTrancada.trancada) {
      const bonus = calcularBonusHabilidade(
        habilidadeTrancada,
        atributos,
        pontosAtributoAncestralidade
                      ...
                      {habilidades.map((hab) => (
                        <tr key={hab.id} className="hover:bg-gray-50">
                          ...
                          <td className="border border-gray-300 p-2 text-center font-bold text-lg">
                            +
                            {bonusHabilidades[hab.id] || // USA O BÔNUS SALVO NO CONTEXTO
                              calcularBonusHabilidade(
                                hab,
                                atributos,
                                pontosAtributoAncestralidade
